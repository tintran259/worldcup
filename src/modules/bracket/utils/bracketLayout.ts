...
export function computeNodePositions(rounds: BracketRound[]): BracketNodePosition[] {
  ...
  rounds.forEach((round, roundIdx) => {
    ...
    round.matches.forEach((match, matchIdx) => {
      ...
      if (roundIdx === 0) {
        // Base column: nodes stacked from top
...
/**
 * Generates a straight segmented (L-shaped elbow) SVG path from the right
 * edge of one node to the left edge of the next.
 *
 * Shape:  ──┐
 *           │
 *           └──
 *
 * The turn happens at the horizontal midpoint of the ROUND_GAP so both
 * upper and lower feeders share the same vertical axis — producing the
 * classic tournament bracket silhouette with crisp right-angle bends.
 */
export function generateConnectorPath(
  ...
  // H → horizontal to midpoint, V → vertical to target Y, H → horizontal to card edge
