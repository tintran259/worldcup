...
import { ROUND_GAP, NODE_WIDTH } from '../types'
...
const C = {
  neutral:      'rgba(203, 213, 225, 0.55)',  // slate-300 — default grid lines
  active:       'rgba(37,  99,  235, 0.95)',  // FIFA blue
  winner:       'rgba(16,  185, 129, 0.95)',  // emerald
  live:         'rgba(239,  68,  68, 0.95)',  // red
...
// ─── Stagger timing ───────────────────────────────────────────────────────────
// Earlier rounds draw first → bracket builds left to right
...
// ─── Framer Motion path draw variant ─────────────────────────────────────────
...
// ─── Inline SVG keyframes ─────────────────────────────────────────────────────

const SVG_STYLES = `
  /* Marching ants on live paths */
  @keyframes bc-flow {
    to { stroke-dashoffset: -22; }
  }
  /* Breathing opacity for live base */
  @keyframes bc-live-breathe {
    0%,100% { stroke-opacity: 0.95; }
    50%      { stroke-opacity: 0.40; }
  }
  /* Winner pulse */
  @keyframes bc-winner-pulse {
    0%,100% { stroke-opacity: 0.90; }
    50%      { stroke-opacity: 0.46; }
  }
  /* Active shimmer dot */
  ...
  /* Joint dot pop on mount */
  @keyframes bc-dot-pop {
    0%   { r: 0;   opacity: 0; }
    60%  { r: 3.2; opacity: 1; }
    100% { r: 2.6; opacity: 1; }
  }
  /* Joint dot pulse for live */
  @keyframes bc-dot-live {
    0%,100% { r: 2.6; opacity: 0.90; }
    50%      { r: 3.8; opacity: 0.55; }
  }
...
// ─── Joint dot helpers ────────────────────────────────────────────────────────
// For each L-shaped path  M x1 y1 H midX V y2 H x2
// the corner point is (midX, y2). We parse it from the `d` string.

function parseJoint(d: string): { x: number; y: number } | null {
  // Path format: "M x1 y1 H midX V y2 H x2"
...
export function SVGConnections({ connectors, width, height }: SVGConnectionsProps) {
  ...
  return (
    ...
    >
      ...
      {/*
        ════════════════════════════════════════════
        LAYER 1 — Glow halos (blurred under-layer)
        ════════════════════════════════════════════
      */}
      <g aria-hidden="true" opacity={0.70}>
        {live.map((c) => (
          <path key={`${c.id}-h`} d={c.d} fill="none"
            stroke={C.glowLive}   strokeWidth={5}
            strokeLinejoin="round" strokeLinecap="round"
            filter={`url(#${uid}-gl)`} />
        ))}
        {winners.map((c) => (
          <path key={`${c.id}-h`} d={c.d} fill="none"
            stroke={C.glowWinner} strokeWidth={4.5}
            strokeLinejoin="round" strokeLinecap="round"
            filter={`url(#${uid}-gw)`} />
        ))}
        {active.map((c) => (
          <path key={`${c.id}-h`} d={c.d} fill="none"
            stroke={C.glowActive} strokeWidth={6}
            strokeLinejoin="round" strokeLinecap="round"
            filter={`url(#${uid}-ga)`} />
        ))}
      </g>
      ...
      {/*
        ════════════════════════════════════════════
        LAYER 2 — Framer Motion path draw
        All connectors animate pathLength 0 → 1.
        Segmented (H V H) paths draw as crisp elbows.
        ════════════════════════════════════════════
      */}
      <AnimatePresence>
        {neutral.map((c) => (
          <motion.path
            key={c.id} d={c.d} fill="none"
            stroke={C.neutral}   strokeWidth={1.2}
        ...
        {winners.map((c) => (
          <motion.path
            key={c.id} d={c.d} fill="none"
            stroke={C.winner}    strokeWidth={1.8}
            strokeLinejoin="round" strokeLinecap="round"
        ...
        {active.map((c) => (
          <motion.path
            key={c.id} d={c.d} fill="none"
            stroke={C.active}    strokeWidth={2.2}
            strokeLinejoin="round" strokeLinecap="round"
            custom={getDelay(c)}
            variants={pathDraw}
            initial="hidden" animate="visible" exit="exit"
      ...
      {/*
        ════════════════════════════════════════════
        LAYER 3 — CSS-only effect overlays
        (Marching ants on live, shimmer on winner/active)
        ════════════════════════════════════════════
      */}
      <g aria-hidden="true">
        {live.map((c) => (
          <path key={`${c.id}-flow`} d={c.d} fill="none"
            stroke={C.liveFlow}   strokeWidth={1.5}
            strokeLinejoin="round" strokeLinecap="round"
            className="bc-live-flow" />
        ))}
        {winners.map((c, i) => (
          <path key={`${c.id}-sh`} d={c.d} fill="none"
            stroke={C.winnerShimmer} strokeWidth={3}
            strokeLinejoin="round" strokeLinecap="round"
        ...
        {active.map((c, i) => (
          <path key={`${c.id}-sh`} d={c.d} fill="none"
            stroke="rgba(147,197,253,0.42)" strokeWidth={2.5}
            strokeLinejoin="round" strokeLinecap="round"
            className="bc-shimmer"
            style={{ animationDelay: `${i * 0.38 + 0.2}s` }} />
        ))}
      </g>

      {/*
        ════════════════════════════════════════════
        LAYER 4 — Corner joint dots
        A small circle at the elbow (midX, toCY) of each L-path.
        These reinforce the segmented-line aesthetic and
        provide a subtle visual anchor at every bracket branch point.
        ════════════════════════════════════════════
      */}
      <g aria-hidden="true">
        {neutral.map((c) => {
