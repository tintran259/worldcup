...
export function useZoomPan({ contentWidth, contentHeight }: UseZoomPanOptions): UseZoomPanReturn {
  ...
  // ── Direct DOM transform (bypass React for every-frame updates) ──────────────
  ...
  // ── Zoom toward a viewport point (cx, cy) ────────────────────────────────────
  const zoomToPoint = useCallback((newZoom: number, cx: number, cy: number) => {
    ...
    const cpx = (cx - px) / z
    const cpy = (cy - py) / z
    // New pan keeps that content point at the same viewport position
    const npx = cx - cpx * cz
    const npy = cy - cpy * cz
    commitTransform(cz, npx, npy)
  }, [commitTransform])

  // ── Fit the entire bracket in the viewport ────────────────────────────────────
  const fitToContainer = useCallback(() => {
    ...
    const fz = Math.min(sx, sy, 1)   // never scale above 100 %
  ...
  // Initial center on first mount (only when panX/Y are both 0)
  useEffect(() => {
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    // ctrl+scroll = pinch-simulate, use finer step
